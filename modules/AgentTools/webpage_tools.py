...
1. Patchright 真实渲染 + 正文提取(主路径):对 JS 渲染/SPA/动态页面有效
...
from modules.site_catch import Catcher, CloudflareChallengeError, is_cloudflare_challenge
...
class WebpageTools(AgentToolBase):
    ...
    async def read_webpage(self, ctx: ToolContext, url: str, goal: str = "提取页面主要内容") -> str:
        ...
        """
        try:
            # 1) Patchright 真实渲染 + 正文提取
            catcher = await Catcher.init()
            title, text = await catcher.catch_text(url)
