...
class GithubTools(AgentToolBase):
    @tool(group="github", preserve=True)
    ...
    async def gh_search_repos(self, ctx: ToolContext, query: str) -> str:
        ...
        lines = []
        for i in items:
            desc = (i.get("description") or "(无描述)").strip()
            lines.append(
    ...
    async def gh_read_file(self, ctx: ToolContext, repo: str, path: str) -> str:
        ...
        try:
            ...
            return f"文件解码失败: {e}"
        return f"文件: {data.get('path')}({data.get('size')} 字节)\n---\n{content}"
