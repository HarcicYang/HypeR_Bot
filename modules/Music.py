...
# The method below was just copied from others :)
def get_music_cloud163(url):
    with open('./lib/163.js') as f:
        jsdata = f.read()
    ...
    html = httpx.post(url=url, data=form1, headers=headers).json()
    mp3_url = html['data'][0]['url']
    return mp3_url


def reformat(file_name: str) -> str:
    if os.path.exists("temp.wav"):
        os.remove("temp.wav")
    if os.path.exists("output.amr"):
        os.remove("output.amr")
    path = os.path.abspath(file_name)
    subprocess.call(f"ffmpeg -i {path} -ar 8000 -ac 1 -acodec pcm_s16le -vn temp.wav")
    subprocess.call(f"ffmpeg -i temp.wav -acodec amr_nb output.amr")
...
class Module(ModuleClass.Module):
    async def handle(self):
        ...
        if cmds.startswith(".song"):
            music_id = cmds.split(" ")[1]
            music_url = f"https://music.163.com/#/song?id={music_id}"
            mp3_url = get_music_cloud163(music_url)
            with open("./music.mp3", "wb") as f:
                f.write(httpx.get(mp3_url).content)
